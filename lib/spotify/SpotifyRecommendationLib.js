...
class SpotifyRecommendationLib {
    ...
    async getNewReleases({ limit, offset, country }) {
        ...
        try {
            const { data } = await axios.get(
                `${config.spotifyApi}/new-releases?${country ? `country=${country}` : ''}&offset=${offset || 0}&limit=${limit || 20}`,
                axiosConfig,
            );
            return data;
        } catch (error) {
    ...
    async getFeaturedPlaylists({ limit, offset, country }) {
        ...
        try {
            const { data } = await axios.get(
                `${config.spotifyApi}/featured-playlists?${country ? `country=${country}` : ''}&offset=${offset || 0}&limit=${limit || 20}`,
                axiosConfig,
            );
            return data;
        } catch (error) {
